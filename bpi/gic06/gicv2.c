...
// default_isr function handler
static void default_isr (int n)
{
  (void) n;
  uart_puts ("unhandled interrupt\n");
}

// init the array of ISR functions
void isrs_init( void )
{
  int i;
...
// configure an interrupt on gicv2 & add the function to ISRS array
